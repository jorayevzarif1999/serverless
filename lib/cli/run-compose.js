...
const relativeBinPath = '@serverless/compose-beta/bin/bin';
...
module.exports = async () => {
  ...
  if (shouldInstallCompose) {
    ...
    try {
      ...
      try {
        await spawn('npm', ['install', '--save-dev', '@serverless/compose-beta']);
      } catch {
        process.stdout.write(
          `${[
            '',
            'Could not install Serverless Compose CLI locally.',
            'Please install it manually with "npm i --save-dev @serverless/compose-beta" and run this command again.',
          ].join('\n')}\n`
        );
        return;
      }
